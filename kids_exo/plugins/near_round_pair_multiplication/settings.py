...
SUPPORTED_FORMATS = {"guided_same_side_round", "expression_with_answer_blank"}
SUPPORTED_STRATEGIES = {"both_below_round", "both_above_round"}


@dataclass(frozen=True)
class NearRoundPairSettings:
...
def load_settings(data: dict) -> NearRoundPairSettings:
    strategies = tuple(data.get("strategies", ["both_below_round", "both_above_round"]))
    raw_weights = data.get("strategy_weights") or {
        strategy: 1.0 for strategy in strategies
    }
    settings = NearRoundPairSettings(
...
def _validate_settings(settings: NearRoundPairSettings) -> None:
    ...
    if len(settings.near_round_distances) < 2 or any(
        distance < 1 or distance > 9 for distance in settings.near_round_distances
    ):
        raise ValueError("At least two near-round distances from 1 to 9 are required")
    if not settings.strategies or set(settings.strategies) - SUPPORTED_STRATEGIES:
        raise ValueError("Unsupported near-round pair strategy")
