...
| Phase 1 当前进展 | 在线 catalog 已扩展为 Subject -> Skill；Math 开放多种速算专项，French 开放 alphabet sounds 与 family common words；session 快照、服务端判分、安全 Student view、locale fallback、`audio_url` 与公开 settings schema/catalog 已建立 |
| Phase 2 当前进展 | FastAPI 已提供 catalog、保存/答题、learner session history、analytics、results 与 printable worksheet API；SQLAlchemy repository 与 Alembic migrations 已覆盖 learner、session lifecycle、question、attempt、timer 与 audio metadata |
| Phase 3 当前进展 | Angular Material 原型已提供 Parent Studio、Learner Management、Learner Detail、Printable Worksheet 与 Student Practice 页面、完成成绩、错题复盘、从错题生成练习、短链接恢复、暂停计时、scratch pad，并使用 lazy feature routes 与 mobile-first 布局 |
...
| 家长结果页 | 查看 learner 的历史摘要、技能统计、错题本入口、近期 session 列表，并可重新打开未完成练习或查看完成结果。 |
| PDF 延续能力 | 家长可通过独立 Printable worksheet 页面生成现有全部 PDF preset，并按次选择 warm-up、页数或自定义题量。 |
...
- 可选 `audio_url`，用于法语等语言题播放稳定的静态音频。
- 服务端标准答案与判分类型。
- 技能标签。
...
孩子可能直接拿手机或平板做练习，因此 Student 端采用 mobile-first 布局。不同视口下不改变题目内容和判分规则，只改变空间组织方式。数学题可以打开 scratch pad，并在 textarea 与 drawing canvas 之间切换；语言类练习默认隐藏 scratch pad，让注意力集中在听音和选择上。
...
语言题还可以携带静态音频资产，例如 `audio_url = "/audio/tts/fr/fr-FR-DeniseNeural/alphabet/a.mp3"`。Student UI 优先播放 `audio_url`，仅在没有静态音频时使用 `speech_text` 与 `speech_locale` 作为浏览器 TTS fallback。这样可以避免 Chrome、Edge、Safari 等浏览器内置 TTS 发音不一致。
...
- Student API 只返回本次 session 已解析好的题面、可选静态音频 URL 和界面所需内容，不返回答案，也不要求浏览器了解 plugin 的翻译文件布局。
...
- 已确认：Parent Studio 使用 Subject -> Skill 两级选择；首批内容从数学速算扩展到法语听音选择。
...
当前已完成首批单项切片：Math 的多种 mental multiplication plugins 与 French 的 `french_alphabet_sounds`、`french_common_word_sounds` 可通过 online catalog 暴露各自设置；Parent UI 根据 schema 仅发送所选 plugin 公开的字段，生成器内部设置不会成为公共请求字段。French 题目可通过 `audio_url` 引用预生成 MP3，并保留浏览器 TTS 作为 fallback。
...
- 已完成首版：Parent 创建或选择 learner，先选择 Subject，再选择 Skill，并配置该题型开放的选项、题量、反馈和计时。
- 已完成首版：Student 以一题一屏完成整数速算 session。
- 已完成首版：Student 可完成法语字母与家庭词汇的听音选择 session，优先播放静态 MP3。
...
- 已完成首版：Student timer 默认开启，可暂停/恢复；刷新或关闭页面时通过后端 timer settlement 和最后一次答题时间兜底。
- 已完成首版：数学题提供可手动打开的 scratch pad，支持 textarea 与 touch-friendly canvas draw mode；语言题隐藏 scratch pad。
